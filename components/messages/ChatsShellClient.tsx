...
export default function ChatsShellClient({
  children,
}: {
    ...
    connectChatSocket(token, {
      onNewMessage: (payload) => {
        ...
        const msg = message as {
          senderId?: { _id?: string; fullName?: string; name?: string };
        };
        if (!cid || !msg) return;
        ...
        if (showNotification && "Notification" in window) {
          ...
          if (Notification.permission === "granted") {
            const senderName =
              msg.senderId?.fullName || msg.senderId?.name || "Someone";
            const body = ((message as { content?: string }).content || "").slice(
              0,
              80
            );
