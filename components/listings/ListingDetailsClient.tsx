...
export function ListingDetailsClient({ listingId }: { listingId: string }) {
  ...
  const { data: relatedRes } = useQuery({
    queryKey: ["listing-related", listingId],
    queryFn: () => listingsAPI.getRelated(listingId, 8),
    enabled: Boolean(listingId) && Boolean(data?.data),
  });
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
            ...
            {listing.images && listing.images.length > 0 ? (
              <>
                <div className="relative aspect-square rounded-lg overflow-hidden bg-white/5">
                  {(() => {
                    const cur = listing.images![currentImageIndex];
                    const mainSrc =
                      (cur as { url?: string })?.url ||
                      (typeof cur === "string" ? cur : "") ||
                      "";
                    if (!mainSrc) {
                      return (
                        <p className="flex h-full items-center justify-center text-white/50">
                          No image
                        </p>
                      );
                    }
        ...
        {/* Internal linking: related sport listings (same sport / area) for crawl paths */}
        {relatedBuckets &&
          (() => {
            ...
            return (
              <section className="mb-12 border-t border-white/10 pt-10" aria-labelledby="related-listings">
                <h2 id="related-listings" className="text-2xl font-bold text-white mb-2">
                  Related listings
                </h2>
                <p className="text-white/60 text-sm mb-6">
                  More {formatSport(listing.sportType || "")} players, teams, tournaments, and grounds you may like.
                </p>
                  ...
                  {merged.map((item) => {
                    ...
                    return (
                      <li key={id}>
                        <Link
                          href={href}
                          className="block rounded-xl border border-white/10 bg-white/5 p-4 hover:border-[#00FFA3]/40 transition-colors"
                        >
                          <span className="text-xs uppercase text-[#00FFA3] font-semibold">
